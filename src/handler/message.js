...
class MessageHandler {
    ...
    async handleMessage(payload, reply) {
        ...
        try {
            ...
            if (postback && postback.payload) {
                ...
                await this.handlePostBack(response.context, {
                    payload: message.text
                }, profile, sender, reply, t);
    ...
    async handleShowAllLanguages(sender, reply, t) {
        ...
        await this.send(reply, {
            'text': `${t.say('ok_here_goes')}\n\n${first.toString().replace(/,/g, ', ')}`
        });

        await this.send(reply, {
            'text': `${second.toString().replace(/,/g, ', ')}`
        });

        return this.send(reply, {
            'text': `${list.toString().replace(/,/g, ', ')}\n\n ${t.say('gasp')}`
        });
    }
...
const getLocale = (profile) => {
    if (!profile || (profile && !profile.locale || profile.locale.indexOf('_') === -1)) {
        return;
    }
